...
class Room():
    def __init__(self,roomID,doorIDs,doorDirections,objects,enterText,investigateText):
        self.roomID = roomID
        self.doors = doorIDs
        self.doorDirections = doorDirections
        self.objects = objects
        self.enterText = enterText
        self.investigateText = investigateText
        
    def enter(self):
        ui.write(self.enterText)
        
    def investigate(self):
        ui.write(self.investigateText)
        
    def moveRoom(self,direction):
        ui.write('You go through %s door'%direction) 
        

roomR1 = Room('R1',['D1','D2',],['north','south'],None,'You find yourself in a large empty room.\nThere are two doors.\n', 'The walls are made of stone, there is one door to the %s and another to the %s'%('north','south'))
...
def act(action):
    go = ['go','enter','travel','walk','move']
    investigate = ['investigate','look','see','find']
    resolved = False
    if not resolved:
        for item in go:
            if item.lower() in action.lower():
            #will make construct to hold current room
                for room in roomR1.doorDirections:
                    if room.lower() in action.lower():
                        resolved = True
                        roomR1.moveRoom(room)
                if not resolved:
                    resolved = True
                    ui.write('That is not a door, maybe look closer.')
